...
`TransactionAwareAsynchronousBus` impl (always-outbox, joins caller's tx) → relay → RabbitMQ adapter (topology from frozen registries at startup). ⛔ OPEN DESIGN QUESTION (asked 1×): what is the outbox PORT in core — core is slf4j-only (no Spring, no JDBC), so what interface must core declare for the Spring module to implement with real SQL? Answer shapes the tx-aware bus the same way MessageTransport shaped the async one. No tx-aware bus code until answered.
